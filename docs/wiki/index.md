...
查看 [常见问题解答](/wiki/FAQs) 以及我们官方文档中的数百个实时示例，网址为：[https://nicegui.io](https://nicegui.io)。
...
| **[NiceGUI WebSerial](https://github.com/zauberzeug/nicegui/discussions/334)** | 使用 JavaScript 的 WebSerial API，通过运行在服务器上的 Python 代码在客户端控制 NeoPixel LED。 |
| **[FIEF Web 应用演示](https://github.com/ClaudioBsh/publicdemos/releases)** | 包含 Traefik、FastAPI、FIEF、PostgreSQL、Redis、HTML、NiceGUI、Bash 脚本、Docker-Compose 的综合演示。 |
| **[NiceGUI CRUD 组件](https://pypi.org/project/niceguicrud/)** | 为您的 Python 对象列表提供用户界面，支持创建、更新和删除项目，可从 Pydantic 的 BaseModel 自动配置。 |
...
| **[Discord Oauth](https://github.com/FlynV/NiceGUI-Discord-Oauth2)** | 使用 NiceGUI 和 Zenora (Discord API 包装器) 实现 Discord Oauth 的简单集成，用于登录和注销。 |
...
| **[Broh5](https://github.com/nghia-vo/broh5)** | 基于浏览器的 HDF5 查看器，采用 MVC 模式实现，并提供单体版本作为对比。 |
...
| **[Reemote](https://reemote.org)** | 用于任务自动化、配置管理和应用程序部署的 Python API。包含图形用户界面，可在多台服务器上安装和配置软件。 |
...
正如在 [#1371](/zauberzeug/nicegui/discussions/1371) 中所讨论的，NiceGUI 目前尚未被 ChatGPT 和其他 LLM 所熟知。这很不幸，因为这常常导致它们胡乱猜测而不是承认自己的无知。

**如何解决：**
我们只需使用 **[这个提示文本](Chat-GPT-4-Prompt)**，将其复制粘贴到 ChatGPT 对话的开头即可。这将为 ChatGPT 提供 NiceGUI 的基本概念。

**进阶资源：**
如果您真的希望 LLM 理解 NiceGUI，我们提供了以下资源：
1.  **完整的 JSON 格式文档**：[https://nicegui.io/static/sitewide_index.json](https://nicegui.io/static/sitewide_index.json)
2.  **自定义指令**：您可以使用一段 [简短文本](ChatGPT-Custom-Instructions) 作为您的 [自定义指令](https://openai.com/blog/custom-instructions-for-chatgpt) 的一部分。
